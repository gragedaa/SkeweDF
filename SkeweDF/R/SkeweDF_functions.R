...
#' @export
psi_criterion_function <- function(params, data, model_fn, pmf_weight = 0, weighted_rt = F, left_trunc = 1, right_trunc = left_trunc + length(data) - 1){
...
#' @export
psi_criterion_RGHD_ratio <- function(params, data, m, pmf_weight = 0, weighted_rt = F, left_trunc = 1, right_trunc = left_trunc + length(data) - 1){ ## multistart can't maximize atm so I just negate the criterion for simple minimization
...
#' @param right_trunc Int used to determine ending index of model to use for optimization
#' @export
local_fit_function <- function(param_bounds, data, model_fn_name, weighted_rt = F, par_chunk = 100, par_chunk_size = 10, n_cores = 1, clust, left_trunc = 1, right_trunc = left_trunc+length(data)-1){
  ...
  parameters <- parLapply(clust,1:par_chunk, function(q){
    par_mat <- lapply(1:skeweDF_gVar_par_chunk_size, function(i){
      ...
      for(i in 1:length(skeweDF_gVar_param_bounds)){
        par_vec[i] <- sample(skeweDF_gVar_param_bounds[[i]], size = 1, replace = T)
      }
      return(par_vec)
  ...
  parameters <- parameter_post_processing(parameters, model_fn_name)
...
#' @param right_trunc Int used to determine ending index of model to use for optimization
#' @export
local_fit_RGHD_ratio <- function(param_bounds, data, weighted_rt = F, par_chunk = 100, par_chunk_size = 10, n_cores = 1, clust, left_trunc = 1, right_trunc = left_trunc+length(data)-1){
  ...
  parameters <- parameter_post_processing(parameters, 'RGHD')
...
#' @param clust socket cluster object from 'parallel::makeCluster()'. This is used if you have already generated a socket cluster object and would like to run this functoin on it. If no object is defined, one will be made for this function call.
#' @export
global_fit_function <- function(param_bounds, data, model_fn_name, iter = 1, weighted_rt = F, n_cores = 1, clust){
...
#' @export
parameter_post_processing <- function(parameter_df, model_fn_name){
...
#' @export
calculate_label_coords <- function(x_lower_bound, x_upper_bound, y_lower_bound, y_upper_bound, x_buffer = 0.5, y_buffer = 0.5, log_scale_x = F, log_scale_y = F){
  output_x <- 0
  output_y <- 0
...
plot_model <- function(title, model_fn_name, data, parameter_df, n_parameters, plot_folder_name, xlab, left_trunc = 1){
  ...
  tmp <- calculate_label_coords(1, length(data), min(data[data != 0]), max(data), x_buffer = 0, y_buffer = 0.25, log_scale_y = T)
  pmf_text_x <- tmp[1]
  pmf_text_y <- tmp[2]
...
write_parameter_table <- function(parameter_df, folder_name, model_fn_name, RGHD_m = 0){
  ...
  write.table(parameter_df, paste0(folder_name,'/',fn_name,'/',fn_name,' optimized_parameters.txt'), sep = '\t', quote = F, row.names = F)
}
...
write_summary_table <- function(parameter_df, folder_name, model_fn_name, RGHD_m = 0){
  ...
  write.table(summary_df, paste0(folder_name,'/',fn_name,'/',fn_name,' parameter_summary.txt'), sep = '\t', quote = F, row.names = F)
}
...
write_input_table <- function(folder_name, data){
  ...
  write.table(input_df, paste0(folder_name,'/input_data.txt'), sep = '\t', quote = F, row.names = F)
}
