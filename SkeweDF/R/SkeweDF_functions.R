...
#' @param m Int used to indicate the number of 'a' terms within the 'param_bounds' variable. The remaining values will be considered 'b' terms.
#' @param data Vector of observed values
#' @param left_trunc Int used to determine starting index of model to use for optimization
#' @param right_trunc Int used to determine ending index of model to use for optimization
#' @param pmf_weight Numeric of weight given to probability mass function for generation of Psi Criterion. For example, if pmf_weight <- 0.5, 50 percent of the Psi Criterion value will be attributed to the probability mass function while the other 50 percent will be attributed to the right-tail cumulative distribution function.
#' @param weighted_rt Boolean used to determine if the weighted right-tail cumulative distribution function should be used or not.
#' @export
psi_criterion_generalized_kw <- function(x, m, data, left_trunc, right_trunc, pmf_weight, weighted_rt){
  pmf <- psi_criterion_generalized_kw_pmf(x, m, data, left_trunc, right_trunc)
  cdf <- psi_criterion_generalized_kw_cdf(x, m, data, left_trunc, right_trunc, weighted_rt)
...
#' @param right_trunc Int used to determine ending index of model to use for optimization
#' @export
psi_criterion_generalized_kw_pmf <- function(x, m, data, left_trunc, right_trunc){
  a <- x[1:m]
  b <- x[(m+1):(length(x)-1)]
  theta <- x[length(x)]
...
#' @param weighted_rt Boolean used to determine if the weighted right-tail cumulative distribution function should be used or not.
#' @export
psi_criterion_generalized_kw_cdf <- function(x, m, data, left_trunc, right_trunc, weighted_rt){
  a <- x[1:m]
  b <- x[(m+1):(length(x)-1)]
  theta <- x[length(x)]
...
#' @param right_trunc Int used to determine ending index of model to use for optimization
#' @export
fit_generalized_kw <- function(param_bounds, m, data, weighted_rt = FALSE, pmf_weight = 0.0, par_chunk = 100, par_chunk_size = 10, n_cores = 1, clust, left_trunc = 1, right_trunc = left_trunc+length(data)-1){
    ...
    if(missing(clust)){
      standalone <- TRUE
      clust <- makeCluster(n_cores)
      clusterExport(clust, varlist = c('data','param_bounds','weighted_rt', 'par_chunk_size','m','left_trunc','right_trunc', 'pmf_weight'), envir = environment())
      clusterCall(clust, function() library(SkeweDF))
    }
    ...
    parameters <- parLapply(clust,1:par_chunk, function(q){
      ...
      fn_parameters <- multistart(parmat = par_mat,fn = psi_criterion_generalized_kw,method = 'L-BFGS-B',
                                  lower = param_lower, upper = param_upper,
                                  data = data, m = m, pmf_weight = pmf_weight, weighted_rt = weighted_rt,
      ...
      for(i in (m+1):length(fn_parameters)){
        colnames(fn_parameters)[i] <- paste0('b',i-m)
      }
    ...
    if(length(1:m) > 1){
      parameters[1:m] <- t(apply(parameters[1:m], 1, FUN=function(x) sort(x)))
    }
    if(length((m+1):(length(parameters)-3)) > 1){
      parameters[(m+1):(length(parameters)-3)] <- t(apply(parameters[(m+1):(length(parameters)-3)], 1, FUN=function(x) sort(x)))
    }
